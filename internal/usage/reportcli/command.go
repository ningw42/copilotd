	...
	if r.OpenAI != nil {
		...
		if r.OpenAI.Total.Turns == 0 {
			...
			for _, row := range r.OpenAI.Rows {
				...
				renderTotal(table, label, strconv.QuoteToASCII(row.Model), row.Total)
			}
			_ = table.Flush()
		}
		fmt.Fprintln(&out, "Model totals")
		table := tabwriter.NewWriter(&out, 0, 4, 2, ' ', 0)
		for _, model := range r.OpenAI.Models {
			renderTotal(table, "Range", strconv.QuoteToASCII(model.Model), model.Total)
		}
		renderTotal(table, "Section total", "", r.OpenAI.Total)
		_ = table.Flush()
	}
	fmt.Fprintln(&out, "\n"+caveat)
	return out.String()
}
func renderTotal(out io.Writer, label, model string, total report.Total) {
	fmt.Fprintf(out, "%s\t%s\t%s\t%s\t%s\t%s\t%s\n", label, model, count(total.Turns), metric(total, "input_tokens"), metric(total, "output_tokens"), metric(total, "cache_write_tokens"), metric(total, "cached_tokens"))
	for _, field := range []struct{ name, label string }{{"cache_write_tokens", "cache write"}, {"cached_tokens", "cache read"}} {
