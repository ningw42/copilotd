...
func render(renderer *lipgloss.Renderer, endpoint string, r report.Report, details bool) (string, error) {
	...
	if r.Pricing == nil {
		...
		fmt.Fprintf(&out, "Pricing: original-provider / per-Turn context tiers / single cache-write rates | snapshot: %s (%s)", r.Pricing.Source, r.Pricing.Version)
	...
	if r.Pricing == nil {
		...
		fmt.Fprintln(&out, "Estimated original-provider cost for persisted best-effort observations using current accepted per-Turn context-tier selection and single cache-write rates; not a Copilot bill; may exclude unpriceable Turns.")
	}
