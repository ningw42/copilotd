...
func TestCommandSumsExactServerAmountsBeforeRoundingPeriodTotals(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !hasTableRow(text, "2026-09-01", "All", "$"+tc.total, "2", "2", "2", "—", "—") {
...
func TestCommandRendersWholeRangeTotalInPeriodColumnAtBottom(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.period, func(t *testing.T) {
			...
			if !hasTableRow(text, "Total", "All", "$0.750", "2", "2", "2", "—", "—") {
				t.Fatalf("missing %s whole-range Total/All row:\n%s", tc.period, text)
			}
			lines := strings.Split(text, "\n")
			for index, line := range lines {
				cells := strings.Split(line, "│")
				if len(cells) >= 3 && strings.TrimSpace(cells[1]) == "Total" && strings.TrimSpace(cells[2]) == "All" &&
					(index+1 >= len(lines) || !strings.HasPrefix(strings.TrimSpace(lines[index+1]), "╰")) {
					t.Fatalf("%s whole-range total is not the final table row:\n%s", tc.period, text)
				}
			}
		})
	}
...
func TestCommandMarksAndExplainsPartialAndEntirelyUnpricedCosts(t *testing.T) {
	...
	for _, want := range [][]string{
		{"2026-09-01", "All", "$0.004*", "6", "2", "2", "—", "—"},
		{"", "a", "$0.004*", "4", "1", "1", "—", "—"},
		{"", "b", "—", "2", "1", "1", "—", "—"},
		{"Total", "All", "$0.004*", "6", "2", "2", "—", "—"},
	} {
	...
	for _, want := range []string{
		"2026-09-01 / All — estimated cost: 1/6 priced stored Turns; unknown_model=1; ambiguous_model=1; missing_rate=1; missing_usage=1; inconsistent_usage=1",
		...
		"Total / All — estimated cost: 1/6 priced stored Turns; unknown_model=1; ambiguous_model=1; missing_rate=1; missing_usage=1; inconsistent_usage=1",
	} {
...
func TestCommandExplainsOlderDaemonWithoutInventingCostCoverage(t *testing.T) {
	...
	for _, want := range [][]string{
		...
		{"2026-09-01", "All", "—", "2", "9,007,199,254,740,993", "12", "—", "6,000*"},
		{"Total", "All", "—", "2", "9,007,199,254,740,993", "12", "—", "6,000*"},
...
func TestCommandPlacesEstimatedCostOnlyInBothPrimarySurfaceTables(t *testing.T) {
	...
	for _, want := range [][]string{
		...
		{"2026-09-01", "All", "$0.003", "1", "12", "9", "2,000", "6,000"},
		{"", "claude-observed", "$0.003", "1", "12", "9", "2,000", "6,000"},
		{"Total", "All", "$0.003", "1", "12", "9", "2,000", "6,000"},
		{"Day", "Model(s)", "Turns", "Thinking", "Cache create 5m", "Cache create 1h"},
		{"Day", "Model(s)", "Est. Cost ($)", "Turns", "Input", "Output", "Cache write", "Cache read"},
		{"2026-09-01", "All", "$0.000", "1", "8,012", "9", "2,000", "6,000"},
		{"", "gpt-observed", "$0.000", "1", "8,012", "9", "2,000", "6,000"},
		{"Total", "All", "$0.000", "1", "8,012", "9", "2,000", "6,000"},
		{"Day", "Model(s)", "Turns", "Reasoning", "Reported total"},
	} {
		if !hasTableRow(text, want...) {
