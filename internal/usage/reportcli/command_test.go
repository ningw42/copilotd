...
func TestCommandRendersServerValuesSafelyAndReturnsOutputFailures(t *testing.T) {
	...
	text := out.String()
	header := "Endpoint: " + server.URL + "\nTimezone: UTC\nRange: 2026-09-01 to 2026-09-02 (exclusive)\nPeriod: day\nQuery time: "
	for _, want := range []string{header, "OpenAI", "9,007,199,254,740,993", "6,000*", "cache read: 1/2 stored Turns", "—", `evil\x1b[31m\n\u202e`, "╭", "╯"} {
		if !strings.Contains(text, want) {
