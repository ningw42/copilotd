...
func RegisterUsage(fs *ff.FlagSet) *UsageFlags {
	...
	specs := []spec[UsageConfig]{
		...
		optionalStringField("timezone", func(c *UsageConfig) **string { return &c.Timezone }, "named Area/City or UTC (omitted: terminal-local Unix or representative Windows CLDR mapping; exact territory then 001; API, empty/custom/unmapped key, disabled DST, missing default, or unloadable mapping requires explicit)"),
