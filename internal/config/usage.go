...
func RegisterUsage(fs *ff.FlagSet) *UsageFlags {
	...
	specs := []spec[UsageConfig]{
		...
		optionalStringField("timezone", func(c *UsageConfig) **string { return &c.Timezone }, "named Area/City or UTC (omitted: terminal-local Unix setting or representative CLDR mapping on native Windows)"),
