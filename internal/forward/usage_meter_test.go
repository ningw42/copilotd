...
const qualifyingBufferedResponse = `{"id":"resp-forward","model":"reported-model","status":"completed","usage":{"input_tokens":12,"output_tokens":6}}`
...
func TestForwardUsageMeterBuffersEveryNonSSEIdentityResponseByPayload(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			bodyText := qualifyingBufferedResponse
			...
			forwarder.Handler(endpoint.OpenAIResponsesHTTP())(recorder, httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(`{}`)))
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			client := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
				...
				return &http.Response{StatusCode: http.StatusOK, Header: header, Body: io.NopCloser(strings.NewReader(qualifyingBufferedResponse)), Request: r}, nil
			})}
			sink := &forwardUsageSink{}
			forwarder := newTestForwarder(readyStub("https://upstream.invalid"), client, time.Second, time.Second, time.Second, time.Second, 1<<20, 1<<20, enabledUsageRegistry(sink))
			...
			if recorder.Code != http.StatusOK || recorder.Body.String() != qualifyingBufferedResponse {
...
func TestForwardUsageMeterClassifiesBufferedReadFailureTimeoutAndCancellation(t *testing.T) {
	...
	t.Run("client cancellation is silent", func(t *testing.T) {
		...
		request := httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(`{}`)).WithContext(ctx)
		writer := &failingResponseWriter{header: make(http.Header)}
		done := make(chan struct{})
		go func() {
			forwarder.Handler(endpoint.OpenAIResponsesHTTP())(writer, request)
			close(done)
		}()
		<-started
		cancel()
...
func TestForwardUsageMeterAloneActivatesBoundedRead(t *testing.T) {
	run := func(enabled bool) (int, string, int) {
		t.Helper()
		client := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
			return &http.Response{StatusCode: http.StatusAccepted, Header: http.Header{"Content-Type": {"application/json"}}, Body: io.NopCloser(strings.NewReader(qualifyingBufferedResponse)), Request: r}, nil
		})}
		sink := &forwardUsageSink{}
	...
	if offStatus != http.StatusAccepted || offBody != qualifyingBufferedResponse || offRecords != 0 {
...
func TestForwardUsageObservationPrecedesDownstreamWriteAndOuterShimFailure(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		return &http.Response{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": {"application/json"}}, Body: io.NopCloser(strings.NewReader(qualifyingBufferedResponse)), Request: r}, nil
	})}
	...
	t.Run("downstream write failure does not withdraw", func(t *testing.T) {
		...
		forwarder.Handler(endpoint.OpenAIResponsesHTTP())(writer, httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(`{}`)))
		if sink.count() != 1 {
...
func TestForwardUsageMeterLeavesRecomputedContentLengthAndExactPayload(t *testing.T) {
	client := &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		return &http.Response{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": {"application/json"}, "Content-Length": {"1"}}, Body: io.NopCloser(strings.NewReader(qualifyingBufferedResponse)), Request: r}, nil
	})}
	sink := &forwardUsageSink{}
	forwarder := newTestForwarder(readyStub("https://upstream.invalid"), client, time.Second, time.Second, time.Second, time.Second, 1<<20, 1<<20, enabledUsageRegistry(sink))
	recorder := newDeadlineRecorder()
	forwarder.Handler(endpoint.OpenAIResponsesHTTP())(recorder, httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(`{}`)))
	if recorder.Body.String() != qualifyingBufferedResponse || recorder.Header().Get("Content-Length") != "113" {
