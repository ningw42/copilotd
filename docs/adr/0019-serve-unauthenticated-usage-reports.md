...
#237; CLI cost presentation remains assigned to #238. Actual native release
certification is recorded per revision on #213, not implied by implementation status. Protocol
...
at every aggregate level, and row/model pricing matches. A new client represents a
wholly absent extension as an older daemon and rejects recognized partial shapes;
unknown additive fields and validated original bytes remain intact. Those
terminal-only native Turns, sum, and coverage accumulations use checked int64
arithmetic. Overflow in independently valid but inconsistent rows fails text
rendering before stdout; JSON still emits its validated original bytes. No Catalog
normalization, Requested-model substitution, HTTP-side pricing, or report-time
