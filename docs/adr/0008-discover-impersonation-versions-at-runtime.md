...
value the operator configures becomes an **embedded floor**, served only when
discovery has not succeeded (cold start offline, air-gapped, or an upstream
hiccup), so behavior is never worse than the previous static defaults. The two
non-version headers (`Copilot-Integration-Id`, `X-GitHub-Api-Version`) remain
static. Nothing is written to disk; the cache lives in memory beside the Copilot
token. This reverses the prior stance — recorded in `config.go` §6.7 — that these
were operator-set "knobs because they are version-sensitive."
...
- **Live overrides that win over discovery**: rejected — an override that beats a
  ...
  removes. Configured values are demoted to the floor; discovery always wins when
  it succeeds.
...
- **Discovery gates `/readyz`**: rejected — coupling uptime to a cosmetic version
  string when the floor already works would make copilotd refuse traffic it could
...
- **Runtime discovery with an embedded floor, memory-only** (chosen): the two
  facts self-update with zero operator action, the floor guarantees a never-worse
  baseline, and the refresh-with-floor mechanism is factored into a generic
  `internal/refresh` primitive.

## Consequences

- The two rotating headers stay current automatically; the embedded floor keeps a
...
- One new outbound dependency is introduced: two public, unauthenticated Microsoft
  ...
  idle-exchange abuse signal ADR-0001 avoided.
- Startup discovery **blocks the first mint** (bounded to 5s) so the first exchange
  already presents current versions; the listener is bound first, so `/healthz`
  and a degraded `/readyz` serve throughout.
...
- The flag surface changes: `--editor-version`, `--editor-plugin-version`, and
  `--copilot-user-agent` are removed in favor of the bare-version floors
  `--vscode-version` and `--plugin-version`; `--impersonation-refresh-interval`
  (default 24h) is added. Pre-1.0, no aliases are kept.
- `internal/refresh` becomes a reusable primitive; issue #53 (the vendored Codex
  `models.json` snapshot) is its next candidate consumer once that issue settles
  its runtime-vs-build-time question.
