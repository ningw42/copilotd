...
- **Component.** `internal/logging` builds one Component-free base carrying the
  ...
  Shared infrastructure keeps its own identity; access is `internal/server`
...
- **Keys.** Every application-supplied top-level key is an exported constant in
  ...
  record-local, and context-carried sites; names stay `snake_case`. The registry
  is a declaration — no allow-list, descriptor table, ownership metadata, typed
  constructors, or validation — so collisions render under native slog
  semantics. `inbound` is the matched `net/http.ServeMux` pattern, absent when
  nothing matched — no sentinel, no raw request path, so no client-controlled
  value enters a record; `route` stays reserved for an exact upstream Route. A
  distinction that matters is an attribute (`failure_class`, `trigger`), never a
  message spelling.
...
correlation, ownership, terminal summary. Nothing semantic is inferred:
...
what a human reads and fixes one line at a time. Today it is folklore: 33
...
- **Convention and review only:** rejected — the current tree is what convention
  produced; an omission is invisible in a diff.
...
- **Derive `component` from the record's caller,** in the handler or at package
  ...
  variant can say `cmd/copilotd`: the frame reads `main.<func>` for any
  `package main`.
...
- Every optional injection disappears: four exported `Logger` fields, two
  ...
  `internal/catalog`'s nil guard. Go cannot forbid an explicitly passed nil; it
  now forbids omission.
- The Component clause is not self-enforcing, so that closed inventory is its
  review gate, in ADR-0012's spirit: a new sink or changed owner is an explicit
  test edit.
- Access is emitted after its handler returns, so a wedged request produces no
  access record; this rule adds no start line, watchdog, or timeout (issue
  #138).
...
The decisions sit on issue #139 and its closed children; the conversion is
designed under #149.
