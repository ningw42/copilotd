...
| Codex catalog reviewer override | [ADR-0005](adr/0005-codex-catalog-reemits-pinned-modelinfo.md) | Removes the accepted official entry's `auto_review_model_override` and optionally injects deployment reviewer routing selected by the served Main-model slug. | `--codex-catalog-enabled` plus `--codex-auto-review-model` / `--codex-auto-review-model-overrides` — **off by default** |
