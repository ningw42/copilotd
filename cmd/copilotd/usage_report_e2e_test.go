...
func TestUsageHelpDescribesNativeSelectionAndPresentation(t *testing.T) {
	...
	for _, want := range []string{"Anthropic and OpenAI Turns", "native Surface selection: all, anthropic, openai", "day, week, month, year", "current month's first day", "next month's first day", "terminal-local Unix setting or representative CLDR mapping on native Windows", "exact non-empty UTF-8 Reported model", "secondary native tables and Pricing-model resolutions", "original validated JSON plus newline"} {
