...
func TestUsageHelpDescribesNativeSelectionAndPresentation(t *testing.T) {
	...
	for _, want := range []string{"Anthropic and OpenAI Turns", "native Surface selection: all, anthropic, openai", "day, week, month, year", "current month's first day", "next month's first day", "representative Windows CLDR mapping", "exact territory then 001", "dynamic-timezone API failure", "empty/custom/unmapped key", "disabled DST", "missing default", "unloadable mapping requires explicit", "exact non-empty UTF-8 Reported model", "secondary native tables and Pricing-model resolutions", "original validated JSON plus newline"} {
