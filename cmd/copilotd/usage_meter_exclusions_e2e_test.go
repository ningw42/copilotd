...
func TestServeLifecycleUsageMeterExcludesCountTokensAndCatalogs(t *testing.T) {
	...
	const models = `{"id":"not-an-inference","type":"message","model":"not-an-inference-model","status":"completed","stop_reason":"end_turn","usage":{"input_tokens":91,"output_tokens":17},"data":[
		{"id":"` + usageMeterCodexModel + `","name":"GPT test","vendor":"OpenAI","model_picker_enabled":true,"supported_endpoints":["/responses"]},
	...
	for _, path := range []string{"/models", "/anthropic/v1/models", "/openai/v1/models", "/openai/v1/models?client_version=fixture"} {
		...
		case "/openai/v1/models":
			if catalog.Object != "list" || len(catalog.Data) != 1 || catalog.Data[0].ID != usageMeterCodexModel || catalog.Data[0].Object != "model" {
				t.Errorf("OpenAI catalog shape/membership = %s", body)
			}
		default:
			if len(catalog.Models) != 1 || catalog.Models[0].Slug != usageMeterCodexModel || len(catalog.Data) != 0 {
